...
class CapTuring:
    ...
    def __init__(self, custom_stop_words=None):
        ...
        Args:
            custom_stop_words (list or str, optional): Custom stop words to use.
                Can be a list of words or a path to a text file containing stop words.
        """
    ...
    def add_stop_words(self, stop_words):
        ...
        Args:
            stop_words (list or str): Either a list of stop words or a path to a text file
                containing stop words (one per line)
        """
        if isinstance(stop_words, str) and os.path.isfile(stop_words):
            ...
            try:
                ...
                print(f"Error loading stop words from {stop_words}: {e}")
        elif isinstance(stop_words, (list, set, tuple)):
            # Add stop words from list/set/tuple
            self.stop_words.update(stop_words)
            print(f"Added {len(stop_words)} custom stop words")
        else:
            print(f"Invalid stop words format: {type(stop_words)}. Expected list, set, tuple, or file path.")
            
        # Update the vectorizer with new stop words
        self.update_vectorizer()
        
        # Reset matrices since vectorizer has changed
        self._invalidate_matrices()
    ...
    def _ensure_vectorized(self):
        ...
        if self.tfidf_matrix is None and self.documents:
            ...
            print(f"Using {len(self.stop_words)} stop words for filtering")
            return labels
    ...
    def compare_word_frequencies(self, n=20, figsize=(15, 8), exclude_stop_words=True):
        ...
        Args:
            ...
            figsize (tuple): Figure size
            exclude_stop_words (bool): Whether to exclude stop words from the visualization
        """
    ...
    def export_stop_words(self, filename):
        ...
        Args:
            filename (str): Path to save the stop words
        """
        try:
